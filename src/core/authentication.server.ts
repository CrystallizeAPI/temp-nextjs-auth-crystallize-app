...
export const AUTH_COOKIE_NAME = 'crystal-app-auth-token';
...
export const createAuthGuard = ({ jwtSecret, crystallizeSignatureSecret }: Deps) => {
    ...
    const decodeCrystallizeSignature = async (signature: string): Promise<CrystallizeSignature> => {
        const payload = (await jwtVerify(signature, new TextEncoder().encode(crystallizeSignatureSecret))).payload as CrystallizeSignature
